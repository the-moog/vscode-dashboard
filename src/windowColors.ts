...
export class SettingsFileDeleter {
  ...
  public dispose() {
    ...
    const cc = JSON.parse(
      JSON.stringify(workspace.getConfiguration('workbench').get('colorCustomizations')),
    );

    const deleteSettingsFileUponExit = JSON.parse(
      JSON.stringify(
        workspace
          .getConfiguration('dashboard.windowColors')
    ...
    if (deleteSettingsFileUponExit) {
      ...
      const aColorWasModified =
        (cc['activityBar.background'] !== this.colors.sideBarColor_dark.hex() &&
          cc['activityBar.background'] !== this.colors.sideBarColor_light.hex()) ||
        (cc['titleBar.activeBackground'] !== this.colors.titleBarColor_dark.hex() &&
          cc['titleBar.activeBackground'] !== this.colors.titleBarColor_light.hex()) ||
        (cc['titleBar.activeForeground'] !== this.colors.titleBarTextColor_dark.hex() &&
          cc['titleBar.activeForeground'] !== this.colors.titleBarTextColor_light.hex());
...
export function activate(context: ExtensionContext) {
  ...
  const extensionTheme = workspace
    .getConfiguration('dashboard.windowColors')
    .get<string>('🌈 Theme');
  let baseColor = workspace.getConfiguration('dashboard.windowColors').get<string>('🌈 BaseColor');
  if (baseColor) {
  ...
  // FIXME: Make auto deletion better
