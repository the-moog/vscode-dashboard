function initProjects() {
  ...
  function onInsideProjectClick(e, projectDiv) {
    ...
    if (onTriggerProjectAction(e.target, dataId)) return;

    var newWindow = e.ctrlKey || e.metaKey || e.button === 1;
    openProject(
      dataId,
      newWindow ? ProjectOpenType.NewWindow : ProjectOpenType.Default
    );
  }
  ...
  function onInsideGroupClick(e, groupDiv) {
    ...
    var action =
      actionDiv != null ? actionDiv.getAttribute("data-action") : null;
    if (!action) return;
  ...
  function onTriggerProjectAction(target, projectId) {
    ...
    if (actionDiv == null) return false;
  ...
  function onContextMenu(e) {
    ...
    if (contextMenuForProject) {
      ...
      contextMenuGroupId = groupIdDiv
        ? groupIdDiv.getAttribute("data-group-id")
        : null;
      if (contextMenuGroupId == null) return;
  ...
  function onProjectContextMenuActionClicked(el) {
    ...
    if (action == null || contextMenuProjectId == null) return;
  ...
  function onGroupContextMenuActionClicked(el) {
    ...
    if (action == null || contextMenuGroupId == null) return;
  ...
  function onMouseEvent(e) {
    if (!e.target || e.target.closest(".disabled")) return;

    var contextMenuElement = e.target.closest(
      "#projectContextMenu [data-action]"
    );
